...
The hardware RTC must have been synchronized before studying. Deck browsing works without a valid clock; import occurs when a deck is selected. Day boundaries and due timestamps use UTC so timezone-setting changes do not alter scheduling. The list initially shows deck names and, when a current cache exists, total card counts. Due and new-card counts appear after a deck has been opened and are updated when a session ends. The due count includes learning and relearning cards waiting for their next step, so leaving a session does not hide its outstanding cards or offer additional new cards in their place.
...
When a study session closes, the current scheduling state is written to a checksummed `.state` snapshot. On the next open, a matching snapshot restores the card states and only journal records appended after it are replayed. A changed CSV cache, truncated or replaced journal, or damaged snapshot makes the reader replay the full journal and recreate the snapshot on session exit. The journal remains the authoritative review history and is the file to back up or restore; snapshots can be deleted safely.
