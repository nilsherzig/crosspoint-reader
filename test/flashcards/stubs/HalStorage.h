...
inline std::map<std::string, unsigned> opensForReadByPath;
inline std::map<std::string, uint64_t> bytesReadByPath;
...
inline void touch(const std::string& path) { ++files.at(path)->time; }
...
class HalFile {
 public:
  ...
  bool getModifyDateTime(uint16_t& date, uint16_t& time) const {
    ...
    date = 1;
    time = static_cast<uint16_t>(node->time);
    return true;
  }
  ...
  bool seek64(const uint64_t offset) { return seekSet(static_cast<size_t>(offset)); }
  int read(void* out, size_t size) {
    ...
    pos += size;
    fake::bytesReadByPath[path] += size;
  ...
  bool truncate(const uint64_t length) {
    if (!node || length > node->bytes.size()) return false;
    node->bytes.resize(static_cast<size_t>(length));
    if (pos > length) pos = static_cast<size_t>(length);
    return true;
  }
};
...
class HalStorage {
 public:
  ...
  bool openFileForRead(const char*, const char* path, HalFile& file) {
    file = open(path);
    if (file) ++fake::opensForReadByPath[path];
    return bool(file);
  }
  ...
  bool openFileForAppend(const char*, const char* path, HalFile& file) {
    if (!exists(path)) fake::add(path, "");
    file = open(path);
    file.pos = static_cast<size_t>(file.fileSize64());
    return true;
  }
